  ...
  registerFull(api) {
    ...
    setTimeout(async () => {
        ...
        if (isEnabled) {
             ...
             try {
                // 1. 获取所有账号 ID
                const accountIds = await ncLocalImPlugin.config.listAccountIds(cfg);
                api.logger?.info(`[LocalIM] 发现账号列表: ${JSON.stringify(accountIds)}`);
                ...
                for (const accountId of accountIds) {
                    ...
                    if (account && account.enabled) {
                        ...
                        // 3. 调用 start
                        // 注意：我们通过 gateway.startAccount 或直接 start
                        const startCtx = {
                            account,
                            cfg,
                            log: api.logger,
